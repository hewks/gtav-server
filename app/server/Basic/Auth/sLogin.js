...
class LoginSingleton extends AbstractAuth {
  async tryLoginWithoutCode(player, obj) {
    ...
    const d = await misc.query(
      `SELECT id, email, password, socialclub FROM users WHERE email = '${data.email}' LIMIT 1`
    );
    if (!d[0]) {
  ...
  async loadAccount(player, id) {
    ...
    player.outputChatBox(
      `${i18n.get("sLogin", "annouceSpawnVehicle", player.lang)}`
    );
    player.outputChatBox(
      `${i18n.get("sLogin", "annouceGlobalChat", player.lang)}`
    );
    player.outputChatBox(
      `${i18n.get("sLogin", "annouceOldUser", player.lang)}`
    );
    player.outputChatBox(
      `${i18n.get("sLogin", "annoucePlayerMenu", player.lang)}`
    );
    const onlinePlayers = mp.players.toArray();
    if (onlinePlayers.length < 30) {
      for (const p of onlinePlayers) {
        p.outputChatBox(
          `[${misc.getTime()}] ${player.name} ${i18n.get(
            "sLogin",
            "connected",
            p.lang
          )}`
  ...
  async tryValidateCodeAndLogIn(player, obj) {
    ...
    if (d[0].password !== pass) {
      player.call("cInjectCef", [
        `app.showCode = false; app.enteredCode = "";`,
      ]);
      return this.showError(player, `Wrong password!`);
    }

...
class RegiserSingleton extends AbstractAuth {
  ...
  trySendCode(player, email) {
    ...
    const lastGetCodeTime = (
      (new Date().getTime() - player.verificationDate) /
      1000
    ).toFixed();
  ...
  async checkUsername(player, obj) {
    ...
    const d = await misc.query(
      `SELECT firstName, lastName FROM users WHERE firstName = '${data.firstName}' AND lastName = '${data.lastName}' LIMIT 1`
    );
    if (d[0]) {
  ...
  async tryCreateAccount(player, obj) {
    ...
    const d = await misc.query(
      `SELECT email FROM users WHERE email = '${data.email}' LIMIT 1`
    );
    if (d[0]) {
  ...
  async createAccount(player, d) {
    ...
    await playerSingleton.createNewUser(
      player,
      d.email,
      d.firstName,
      d.lastName,
    ...
    player.call("cInjectCef", [
      `app.showInfo('Success! Now you can log in.');`,
    ]);
  }
}

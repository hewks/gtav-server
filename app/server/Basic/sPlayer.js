...
class PlayerSingleton {
  async createNewUser(player, email, firstName, lastName, pass) {
    ...
    await misc.query(`INSERT INTO users 
      ...
      player.ip
    }', '${new Date().toLocaleString()}', '${JSON.stringify(firstSpawn)}', '${
      player.socialClub
    }')`);
  ...
  loadPlayerTemplate(player) {
    ...
    player.showLoyalityNotification = function (value) {
      ...
      if (value > 0) plus = "+";
      this.notify(
        `${i18n.get("sLoyality", "loyality", this.lang)} ~b~${plus}${value}`
      );
    };
    ...
    player.saveBasicData = function () {
      ...
      misc.query(
        `UPDATE users SET ip = '${
          this.ip
        }', logdate = '${new Date().toLocaleString()}', position = '${JSON.stringify(
          pos
        )}', health = '${this.health}', loyality = '${
          this.loyality
        }' WHERE id = '${this.guid}'`
      );
    };
...
mp.events.addCommand({
  ...
  pos: (player) => {
    ...
    const str = `x: ${misc.roundNum(pos.x, 3)}, y: ${misc.roundNum(
      pos.y,
      3
    )}, z: ${misc.roundNum(pos.z, 3)}, rot: ${misc.roundNum(rot, 2)}`;

...
for (let name of vehicles) {
  ...
  let vehicleName = mp.game.ui.getLabelText(
    mp.game.vehicle.getDisplayNameFromVehicleModel(vehicleHash)
  );
  let vehicleItem = new UIMenuItem(name, "");
  vehicleItem.SetRightLabel(vehicleName == "NULL" ? "" : vehicleName);
  categoryMenus[mp.game.vehicle.getVehicleClassFromName(vehicleHash)].AddItem(
    vehicleItem
  );
}
...
// spawn by model
mainMenu.AddItem(
  new UIMenuItem("Spawn by Model", "Write a vehicle model name to spawn it.")
);

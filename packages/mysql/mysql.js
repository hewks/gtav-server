  ...
  switch (state) {
          ...
          function (err, res) {
            if (res.length > 0) {
              ...
              bcrypt.hash(pass, null, null, function (err, hash) {
                if (!err) {
                  gm.mysql.handle.query(
                    "INSERT INTO `accounts` SET username = ?, password = ?",
                    [username, hash],
                    function (err, res) {
                      if (!err) {
                        player.name = username;
    ...
    default: {
      player.outputChatBox(
        "An error has occured, please contact your server administrator."
      );
      console.log(
        "\x1b[31m[ERROR] Login/Register state was one that isn't defined. State: " +
          state
      );
      break;
    }
